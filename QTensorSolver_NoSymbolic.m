function QTensorSolver_NoSymbolic()
%=======================================================================%
...
N_X = 1000;    N_Y = 1000;    N_Z = 500;

% Iteration
max_iter = 100000000;  % as in your script
tol      = 1e-9;

% Pattern parameters (unchanged)
Lambda = 6.5e-6;

% Visualization flags (kept off for performance/HPC)
plot_result = 0;
wait_bar    = 0;

% Optional: enforce MAT-file version
% save_version = '-v7';
...
alpha = [];  % will be set on iter==1 as in your old code
...
for iter = 1:max_iter
    ...
    for m=1:3, for n=1:3
        % D_n Q_{m,α} for α = x(1),y(2),z(3)
        % α=1 -> component (m,1), take derivative along n
        Gx{m,n} = Gx{m,n} + L3 * Dn_of_Q(fd, n, Q{m,1}, dQx{m,1}, dQy{m,1}, dQz{m,1});
        Gy{m,n} = Gy{m,n} + L3 * Dn_of_Q(fd, n, Q{m,2}, dQx{m,2}, dQy{m,2}, dQz{m,2});
        Gz{m,n} = Gz{m,n} + L3 * Dn_of_Q(fd, n, Q{m,3}, dQx{m,3}, dQy{m,3}, dQz{m,3});
    end, end
    ...
    % Build ∂_β Q_{ij} caches for all i,j,β
    % G part first
    for m=1:3, for n=1:3
        % Gx: α=1
        tmp = zeros(N_Y,N_X,N_Z);
...
function DnQ = Dn_of_Q(fd, n, Qij, dQx_ij, dQy_ij, dQz_ij)
